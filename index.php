...
<body>
  <div class="container-fluid">
    ...
    <!-- Sidebar -->
      <?php
// Gunakan koneksi yang sudah ada dan user_id dari session
$query = "SELECT nama, foto FROM user WHERE id = ?";
...
?>
  ...
  <div class="d-flex align-items-center mb-3" style="margin-top: -10px;">
    ...
    <!-- Logout tetap aman -->
<a href="#" style="background:red; color:white; padding:6px; border-radius:6px; text-decoration:none;" onclick="return konfirmasiLogout(event)">Keluar</a>
...
// Misalnya ambil user_id dari session
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
$user_id = $_SESSION['user_id'] ?? 0; // pastikan session user_id sudah diset saat login

// Hitung total uang masuk untuk user tertentu
$q_masuk = mysqli_query($conn, "SELECT SUM(nominal) as total_masuk FROM uang_masuk WHERE user_id = $user_id");
$totalMasuk = mysqli_fetch_assoc($q_masuk)['total_masuk'] ?? 0;

// Hitung total uang keluar untuk user tertentu
$q_keluar = mysqli_query($conn, "SELECT SUM(nominal) as total_keluar FROM uang_keluar WHERE user_id = $user_id");
...
$user_id = $_SESSION['user_id']; // ⬅️ Ambil user yang sedang login

// Ambil hanya kategori milik user ini
$result = mysqli_query($conn, "SELECT * FROM kategori_batas WHERE user_id = $user_id ORDER BY nama_kategori ASC");
?>
